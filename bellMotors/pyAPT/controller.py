...
class Controller(object):
  ...
  def set_backlash_dist(self, distance, channel=1):
      """Created by Collin.
      Backlash correction number found in APT User"""
      self.backlash = distance
      backlash_APT = int(self.backlash * self.position_scale)
      params = st.pack( '<Hi', channel, backlash_APT)
      movemsg = Message(message.MGMSG_MOT_SET_GENMOVEPARAMS,data=params)
  ...
  def goto(self, abs_pos_mm, channel=1, wait=True):
    ...
    if wait:
      ...
      while moving:
          ...
          if position >= abs_pos_mm-0.009 and position <= abs_pos_mm+0.009:
              moving = False
              break
          else:
              time.sleep(0.5)
      ...
      while sts.velocity_apt:
        #print("Vel: ", sts.velocity_apt)
        time.sleep(0.01)
        sts = self.status()
      #time.sleep(0.5)
      #position = self.position()
      #print("Move complete. Pos: %r, Vel: %r" % (position, sts.velocity_apt))
      return sts
    else:
      return None

  def move(self, step, channel=1, wait=True):
    """
    @author: Collin Schlager
    moves device in a relative manner, given by step
    + forces a CW movement 
    or 
    - forces a CCW movement
    """

    abs_pos_apt = int(step * self.position_scale)
