...
class APTMotor():
    def __init__(self, SerialNum=None, HWTYPE=31):
        ...
        self.aptdll = windll.LoadLibrary(dllPath + '\APT.dll')
    ...
    def initializeHardwareDevice(self):
        ...
        if result == 0:
            self.Connected = True
            if DEBUG: print('initializeHardwareDevice connection SUCCESS')
    ...
    def mHome(self):
        ...
        '''
        #  Added by Saewoo, 2015, July 24
        if DEBUG: print('mHome ')
        if not self.Connected:
            raise Exception('Please connect first! Use initializeHardwareDevice')
        self.aptdll.MOT_MoveHome(self.SerialNum, True)
        if DEBUG: print('mHome SUCCESS')
        return True
